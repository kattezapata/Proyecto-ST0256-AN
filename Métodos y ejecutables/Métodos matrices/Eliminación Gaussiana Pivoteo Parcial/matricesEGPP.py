...
def open_window2():
  try:
    ...
    global xvalues 
    xvalues =  EGPP.EGPP(M,vec)
    etiqueta.config(text="Gaussian elimination with partial pivot done")
...
app = Tk()
app.config(width=1200, height=800)
app.title("Gaussian elimination with partial pivot")
...
bot= Button(vp, text="OK!", command=enter)
