...
def full_pivot_click():
  try:
    m_full_pivot.withdraw()
    global matriz 
    matriz = Toplevel()
    global values
    values = []
    ...
    global vector
    vector = []
    etiqueta1 = Label(vp, text="Vector independiente")
...
def open_window2():
  try:
    ...
    global xvalues 
    xvalues =  full_pivot.full_pivot(np.array(M),np.array(vec))
   ...
   # bot_U.grid(column=N, row=N)
    ...
    tkinter.messagebox.showerror(title="Error", message="Plese input rigth values")
...
m_full_pivot = Tk()
m_full_pivot.config(width=1200, height=800)
m_full_pivot.title("gaussian elimination with full pivot Method")

#Main window
vp = Frame(m_full_pivot)
...
bot= Button(vp, text="OK!", command=full_pivot_click)
